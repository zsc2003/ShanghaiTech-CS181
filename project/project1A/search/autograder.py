...
ERROR_HINT_MAP = {
  ...
  'q7': {
      "<class 'util.TimeoutFunctionException'>": """
        We noticed that your project threw an TimeoutFunctionException on q7.
        While many things may cause this, it may have been from the weak 
        heuristic function. You need to balance the time cost on both calculating
        heuristic function and searching. Try using problem.heuristicInfo as a
        cache (see the docstring of 'foodHeuristic') and submit again!
    """
  }
}
...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP,
    ...
    grades.grade(sys.modules[__name__], exceptionMap=exceptionMap, bonusPic = projectParams.BONUS_PIC)
